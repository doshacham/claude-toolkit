...
A personal collection of Claude Code slash commands, skills, plugins, and autonomous AI coding loops for software engineering workflows. Covers everything from systematic debugging to PRD breakdowns, codebase synthesis, dev-blog generation, team orchestration, and Ralph-style autonomous build loops.
...
│   ├── triagger/
│   └── write-a-skill/
└── loops/             # Autonomous AI coding loops (Ralph methodology + variants)
    ├── how-to-ralph-wiggum/   # Complete Ralph Playbook guide (by ClaytonFarr)
    ├── ralph-loop/            # Multi-phase Ralph loop (Windows/Max plan, sentinel watcher)
    ├── claude-loop/           # Extended loop variant (fix, audit, specs prompts)
    └── ralph-wiggum-files/    # Refined Ralph prompt templates (API mode, Opus/Sonnet)
```
...
Autonomous AI coding loops based on the [Ralph methodology](https://ghuntley.com/ralph/) by Geoff Huntley. Each loop runs Claude in a continuous bash loop, using file-based state (`IMPLEMENTATION_PLAN.md`, `AGENTS.md`, `specs/*`) to maintain context across iterations.

### How-to-Ralph-Wiggum (Reference Guide)

**Directory:** [`loops/how-to-ralph-wiggum/`](loops/how-to-ralph-wiggum/)

ClaytonFarr's comprehensive Ralph Playbook. The definitive breakdown of the Ralph methodology: workflow phases, key principles, loop mechanics, file structure, and proposed enhancements.

| File | Description |
|---|---|
| [`README.md`](loops/how-to-ralph-wiggum/README.md) | Full playbook: 3 phases (requirements, planning, building), key principles (context management, backpressure, "let Ralph Ralph"), loop mechanics, file templates, and enhancement proposals |
| [`index.html`](loops/how-to-ralph-wiggum/index.html) | Styled HTML version of the playbook for web viewing |
| [`files/loop.sh`](loops/how-to-ralph-wiggum/files/loop.sh) | Enhanced loop script with plan/build mode selection and max-iterations support (API mode with `-p` flag) |
| [`files/PROMPT_build.md`](loops/how-to-ralph-wiggum/files/PROMPT_build.md) | Build mode prompt: study specs, pick task, implement, test, commit, push. Guardrails via 999... numbering. |
| [`files/PROMPT_plan.md`](loops/how-to-ralph-wiggum/files/PROMPT_plan.md) | Plan mode prompt: gap analysis (specs vs code), generate/update IMPLEMENTATION_PLAN.md. No implementation. |
| [`files/AGENTS.md`](loops/how-to-ralph-wiggum/files/AGENTS.md) | Template for operational guide: build/run commands, validation commands, codebase patterns |
| [`files/IMPLEMENTATION_PLAN.md`](loops/how-to-ralph-wiggum/files/IMPLEMENTATION_PLAN.md) | Placeholder — generated by Ralph during planning mode |
| [`references/sandbox-environments.md`](loops/how-to-ralph-wiggum/references/sandbox-environments.md) | Comparison of sandbox environments (Sprites, E2B, Modal, Cloudflare, Daytona, Cloud Run, Replit, Docker) for running autonomous agents safely |
| [`references/nah.png`](loops/how-to-ralph-wiggum/references/nah.png) | Geoff Huntley's "nah" tweet screenshot |
| [`references/ralph-diagram.png`](loops/how-to-ralph-wiggum/references/ralph-diagram.png) | Ralph workflow diagram |

### Ralph Loop (Windows/Max Plan)

**Directory:** [`loops/ralph-loop/`](loops/ralph-loop/)

Multi-phase Ralph loop adapted for Windows 11 + Git Bash + Max plan (no API credits). Uses sentinel file + PowerShell background watcher to handle Claude process lifecycle. Three distinct phases with dedicated scripts.

| File | Phase | Description |
|---|---|---|
| [`HOW-TO-USE.txt`](loops/ralph-loop/HOW-TO-USE.txt) | — | Step-by-step usage guide: Phase 1 (specs interview) → Phase 2 (planning loop) → Phase 3 (building loop) |
| [`note.txt`](loops/ralph-loop/note.txt) | — | Technical notes: how the sentinel watcher works, differences from original Ralph (no `-p` flag, PowerShell watcher, Opus subagents) |
| [`README.md`](loops/ralph-loop/README.md) | — | Same as how-to-ralph-wiggum README (full playbook reference) |
| [`loop-specs.sh`](loops/ralph-loop/loop-specs.sh) | 1 | Requirements interview script — runs once, interactive. Produces AUDIENCE_JTBD.md + specs/*.md |
| [`PROMPT_specs.md`](loops/ralph-loop/p1/PROMPT_specs.md) | 1 | Interview prompt: systematically explore audience, JTBDs, topics, acceptance criteria, edge cases via AskUserQuestion |
| [`loop-plan.sh`](loops/ralph-loop/loop-plan.sh) | 2 | Planning loop with sentinel watcher. Generates IMPLEMENTATION_PLAN.md from specs vs code gap analysis |
| [`PROMPT_plan.md`](loops/ralph-loop/PROMPT_plan.md) | 2 | Planning prompt: Opus subagents for code study, acceptance-driven test derivation, non-deterministic evaluation patterns |
| [`loop-build.sh`](loops/ralph-loop/loop-build.sh) | 3 | Build loop with sentinel watcher. Also supports `plan-work` mode for scoped feature branches |
| [`PROMPT_build.md`](loops/ralph-loop/PROMPT_build.md) | 3 | Build prompt: implement from plan, TDD with acceptance criteria tests, mandatory stop after commit+push |
| [`PROMPT_plan_work.md`](loops/ralph-loop/PROMPT_plan_work.md) | 3 | Scoped planning for work branches — plans only tasks related to `${WORK_SCOPE}` |
| [`AGENTS.md`](loops/ralph-loop/AGENTS.md) | all | Template: build/run commands, validation, operational notes |
| [`AUDIENCE_JTBD.md`](loops/ralph-loop/AUDIENCE_JTBD.md) | 1 | Template: audience and jobs-to-be-done |
| [`IMPLEMENTATION_PLAN.md`](loops/ralph-loop/IMPLEMENTATION_PLAN.md) | 2-3 | Placeholder — generated by Ralph |
| `p1/`, `p2/`, `p3/` | 1-3 | Phase-specific copies of prompts and scripts for isolated use |
| [`references/ralph-diagram.png`](loops/ralph-loop/references/ralph-diagram.png) | — | Ralph workflow diagram |

### Claude Loop (Extended Variant)

**Directory:** [`loops/claude-loop/`](loops/claude-loop/)

Extended loop variant with additional prompt modes: fix (targeted bug fixing), audit, and specs interview. Uses same sentinel watcher pattern as ralph-loop.

| File | Description |
|---|---|
| [`loop.sh`](loops/claude-loop/loop.sh) | Generic loop script (plan/build mode selection) |
| [`loop-build.sh`](loops/claude-loop/loop-build.sh) | Build loop with sentinel watcher |
| [`loop-plan.sh`](loops/claude-loop/loop-plan.sh) | Planning loop |
| [`loop-fix.sh`](loops/claude-loop/loop-fix.sh) | Fix loop — targeted bug fixing mode |
| [`loop-specs.sh`](loops/claude-loop/loop-specs.sh) | Requirements interview |
| [`PROMPT_build.md`](loops/claude-loop/PROMPT_build.md) | Build mode prompt |
| [`PROMPT_plan.md`](loops/claude-loop/PROMPT_plan.md) | Planning mode prompt |
| [`PROMPT_plan_work.md`](loops/claude-loop/PROMPT_plan_work.md) | Scoped work-branch planning prompt |
| [`PROMPT_fix.md`](loops/claude-loop/PROMPT_fix.md) | Fix mode prompt — targeted debugging |
| [`PROMPT_fix_plan.md`](loops/claude-loop/PROMPT_fix_plan.md) | Fix planning prompt |
| [`PROMPT_specs.md`](loops/claude-loop/PROMPT_specs.md) | Specs interview prompt |
| [`PROMPT_audits.md`](loops/claude-loop/PROMPT_audits.md) | Audit mode prompt |
| [`DEBUG_LOGGIN.md`](loops/claude-loop/DEBUG_LOGGIN.md) | Debug logging reference |
| [`AGENTS.md`](loops/claude-loop/AGENTS.md) | Operational guide template |
| [`AUDIENCE_JTBD.md`](loops/claude-loop/AUDIENCE_JTBD.md) | Audience/JTBD template |
| [`IMPLEMENTATION_PLAN.md`](loops/claude-loop/IMPLEMENTATION_PLAN.md) | Generated plan placeholder |
| [`note.txt`](loops/claude-loop/note.txt) | Technical notes |
| [`prompt.txt`](loops/claude-loop/prompt.txt) | Additional prompt reference |
| [`HOW-TO-USE.txt`](loops/claude-loop/HOW-TO-USE.txt) | Usage guide |

### Ralph Wiggum Files (Refined Templates)

**Directory:** [`loops/ralph-wiggum-files/`](loops/ralph-wiggum-files/)

Refined Ralph prompt templates for API mode (`-p` flag) with explicit Sonnet/Opus subagent naming and Ultrathink. These are the polished versions ready to drop into any project.

| File | Description |
|---|---|
| [`loop.sh`](loops/ralph-wiggum-files/loop.sh) | Loop script: plan/build mode, max-iterations, API mode with `-p --dangerously-skip-permissions` |
| [`PROMPT_build.md`](loops/ralph-wiggum-files/PROMPT_build.md) | Build prompt: up to 500 Sonnet subagents for reads, Opus for reasoning, Ultrathink for specs fixes |
| [`PROMPT_plan.md`](loops/ralph-wiggum-files/PROMPT_plan.md) | Plan prompt: up to 250 Sonnet subagents for spec study, Opus for analysis and prioritization |
| [`AGENTS.md`](loops/ralph-wiggum-files/AGENTS.md) | Operational guide template |
| [`IMPLEMENTATION_PLAN.md`](loops/ralph-wiggum-files/IMPLEMENTATION_PLAN.md) | Generated plan placeholder |
...
| Run an autonomous build loop | `loops/ralph-wiggum-files/` or `loops/ralph-loop/` |
| Understand the Ralph methodology | `loops/how-to-ralph-wiggum/README.md` |
| Run a loop on Windows/Max plan | `loops/ralph-loop/` (sentinel watcher, no API) |
| Run a loop with fix/audit modes | `loops/claude-loop/` |
...
| **Autonomous Loops** | `how-to-ralph-wiggum` (guide), `ralph-loop` (Windows/Max), `claude-loop` (extended), `ralph-wiggum-files` (refined templates) |
