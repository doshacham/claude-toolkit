...
Investigate the reported problem, find its root cause, and create a GitHub issue with a TDD fix plan. This is a mostly hands-off workflow - minimize questions to the user.

## Process

### 1. Pre-flight

Run `bash ${CLAUDE_PLUGIN_ROOT}/scripts/preflight.sh` to verify the environment. If it fails, stop and report what's missing.

### 2. Capture the problem

The problem description is: **$ARGUMENTS**

If `$ARGUMENTS` is empty, ask ONE question using AskUserQuestion: "What's the problem you're seeing?"
